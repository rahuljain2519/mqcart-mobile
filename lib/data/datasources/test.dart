...
class ShopRemoteDS {
  ...
  /// Create shop (SHOP IS ALWAYS CREATED INACTIVE)
  Future<String> createShop(ShopModel shop) async {
    final data = shop.toJson();

    /// 🔒 FORCE SAFE DEFAULTS (DO NOT BREAK EXISTING FLOW)
    data.addAll({
      'isActive': false,                 // activation controlled by payment
      'activationStatus': 'pending',     // pending | active
      'plan': data['plan'] ?? 'normal',  // default plan
      'productLimit': data['productLimit'] ?? 20,
